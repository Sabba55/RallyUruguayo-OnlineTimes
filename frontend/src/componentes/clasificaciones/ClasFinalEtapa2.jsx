...
import '../../estilos/clasificaciones/ClasFinalEtapa2.css';
import ErrorDisplay from '../errores/ErrorDisplay';

function ClasFinalEtapa2({ onVolver }) {
  ...
  const cargarDatos = async (esInicial = false) => {
    try {
      ...
      const clasificadosPorClase = generarClasificacionEtapa2PorClases(
        datosTiempos,
        datosTramos,
        datosPenalizaciones,
        datosInscriptos
      ...
      console.error(err);
      setError('Error al cargar los datos de clasificación final de Etapa 2 por clases');
      if (esInicial) setCargandoInicial(false);
    }
  };
  ...
  const obtenerPEsEtapa2 = (tramosData = tramos) => {
    return tramosData
      .filter(tramo => {
        const esEtapa2 = tramo.etapa === '2' || tramo.etapa === 2;
        const noCancelado = !estaTramoCancelado(parseInt(tramo.pe), tramosData);
        return esEtapa2 && noCancelado;
      })
      .map(tramo => parseInt(tramo.pe))
      .sort((a, b) => a - b);
  };
  ...
  const obtenerPenalizacionesPiloto = (nroPiloto, tramosData = tramos) => {
    const pesEtapa2 = obtenerPEsEtapa2(tramosData);
    const peMaximoEtapa2 = Math.max(...pesEtapa2);
    
    return penalizaciones.filter(pen => {
      const nroMatch = String(pen.nro).trim() === String(nroPiloto).trim();
      const peOcurrido = parseInt(pen.peocurrido) || 0;
      return nroMatch && peOcurrido <= peMaximoEtapa2;
    });
  };

  const calcularTotalPenalizaciones = (nroPiloto, tramosData = tramos) => {
    const penalizacionesPiloto = obtenerPenalizacionesPiloto(nroPiloto, tramosData);
    const pesEtapa2 = obtenerPEsEtapa2(tramosData);
    let totalSegundos = 0;
    
    penalizacionesPiloto.forEach(pen => {
      const peOcurrido = parseInt(pen.peocurrido) || 0;
      if (pesEtapa2.includes(peOcurrido)) {
  ...
  const pilotoCompletoTodosLosPEsEtapa2 = (piloto, tramosData = tramos) => {
    const pesEtapa2 = obtenerPEsEtapa2(tramosData);
    
    for (let pe of pesEtapa2) {
      const key = `pe${pe}`;
  ...
  const calcularTiempoNetoTotalEtapa2 = (piloto, tramosData = tramos) => {
    const pesEtapa2 = obtenerPEsEtapa2(tramosData);
    let total = 0;
  ...
  const calcularTotalConPenalizaciones = (piloto, tramosData = tramos) => {
    const tiempoNeto = calcularTiempoNetoTotalEtapa2(piloto, tramosData);
    const penalizacionesTotal = calcularTotalPenalizaciones(piloto.nro, tramosData);
  ...
  const obtenerDistanciaTotalEtapa2 = (tramosData = tramos) => {
    const pesEtapa2 = obtenerPEsEtapa2(tramosData);
  ...
  const calcularVelocidadPromedioEtapa2 = (totalSegundos, tramosData = tramos) => {
    if (totalSegundos === 0) return '-';
    
    const distancia = obtenerDistanciaTotalEtapa2(tramosData);
  ...
  const generarClasificacionEtapa2PorClases = (tiemposData, tramosData, penalizacionesData, inscriptosData) => {
    const clasificados = tiemposData
      .filter((p) => {
        // Verificar que completó todos los PEs válidos de la Etapa 2
        if (!pilotoCompletoTodosLosPEsEtapa2(p, tramosData)) return false;
      ...
      .map((p) => {
        ...
        return {
          ...
          totalSegundos: calcularTotalConPenalizaciones(p, tramosData),
          tiempoNetoSegundos: calcularTiempoNetoTotalEtapa2(p, tramosData),
          penalizacionesSegundos: calcularTotalPenalizaciones(p.nro, tramosData),
  ...
  const obtenerNombreCategoriaCorto = (clase) => {
    // Para los botones del índice, usar versión corta
    return clase;
  };
  ...
  return (
    <div className="contenedor-clas-final-etapa2">
          ...
          {ordenarCategorias(Object.keys(pilotosPorClase)).map((clase) => {
            ...
            return (
              <div key={clase} className="mb-5" id={`categoria-${clase}`}>
                <h3 className="text-center encabezado-clase-etapa2">{obtenerNombreCategoria(clase)}</h3>
                
                <div className="tabla-clase-etapa2-wrapper">
                  <table className="table table-bordered tabla-clase-etapa2 mb-0">
                    <thead>
                      <tr className="tabla-encabezado-clase-etapa2 text-center">
                        <th>POS</th>
                        <th>Nº</th>
                        <th>NAC</th>
                      ...
                      {pilotos.length === 0 ? (
                        <tr>
                          <td colSpan="10" className="text-center text-muted py-3">
                            No hay pilotos que hayan completado todos los tramos válidos de la Etapa 2
                          </td>
                        </tr>
                      ) : (
                        pilotos.map((p, i) => {
                          ...
                          return (
                            <tr key={p.nro} className="tabla-fila-datos-clase-etapa2">
                              <td className="text-center">
                                <span className="badge-pos-clase-etapa2">{i + 1}</span>
                              </td>
                              <td className="text-center fw-bold">{p.nro}</td>
                              <td className="text-center celda-nacionalidad-clase-etapa2">
                                {(() => {
                                  ...
                                  return (
                                    <div className="contenedor-banderas-apiladas-etapa2">
                                      <img
                                        ...
                                        alt={nacPiloto}
                                        className="bandera-nacionalidad-clase-etapa2"
                              ...
                              <td className="text-center celda-piloto-clase-etapa2">
                                <div className="nombre-piloto-clase-etapa2">{p.piloto}</div>
                                <div className="nombre-navegante-clase-etapa2">{p.navegante}</div>
                              </td>
                              <td className="text-center celda-vehiculo-clase-etapa2">
                                {(() => {
                                  ...
                                  return (
                                    <>
                                      <img
                                        src={rutaLogo}
                                        alt={marca}
                                        className="logo-marca-clase-etapa2"
                                        onError={(e) => {
                                          e.target.style.display = 'none';
                                        }}
                                      />
                                      <span className="texto-vehiculo-clase-etapa2">{p.vehiculo}</span>
                                    </>
                                  );
                                })()}
                              </td>
                              ...
                              <td className="text-center fw-bold">
                                {calcularVelocidadPromedioEtapa2(p.totalSegundos, tramos)}
                              </td>
                            </tr>
                          );
                        })
...
export default ClasFinalEtapa2;
