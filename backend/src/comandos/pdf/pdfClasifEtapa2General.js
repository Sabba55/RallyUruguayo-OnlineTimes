...
// ─── LÓGICA DE CLASIFICACIÓN (idéntica al frontend ClasFinalEtapa2, general) ─
...
/**
 * Solo PEs con etapa === '2' que no estén cancelados
 */
function obtenerPEsEtapa2(tramos) {
  return tramos
    .filter(t => {
      const esEtapa2    = t.etapa === '2' || t.etapa === 2;
...
function pilotoCompletoEtapa2(piloto, tramos) {
  for (const pe of obtenerPEsEtapa2(tramos)) {
...
function calcularTiempoNeto(piloto, tramos) {
  return obtenerPEsEtapa2(tramos).reduce((sum, pe) => {
    const t = piloto[`pe${pe}`];
...
/**
 * Penalizaciones solo de PEs de Etapa 2
 */
function calcularTotalPenalizaciones(nro, tramos, penalizaciones) {
  const pesEtapa2   = obtenerPEsEtapa2(tramos);
  const peMaxEtapa2 = pesEtapa2.length ? Math.max(...pesEtapa2) : 0;

  return penalizaciones
    .filter(pen => {
      const match   = String(pen.nro).trim() === String(nro).trim();
      const peOcurr = parseInt(pen.peocurrido) || 0;
      return match && peOcurr <= peMaxEtapa2 && pesEtapa2.includes(peOcurr);
    })
...
/**
 * Genera la clasificación general de Etapa 2
 * Solo pilotos que finalizaron (finalizo = SI) y completaron todos los PEs de Etapa 2
 */
function generarClasificacionEtapa2General(tiempos, tramos, penalizaciones, inscriptos) {
  const clasificados = tiempos
    .filter(p => {
      if (!pilotoCompletoEtapa2(p, tramos)) return false;
...
function construirColumnas(doc, pilotos) {
  ...
  const fijos  = ANCHO_POS + ANCHO_NRO + ANCHO_NAC + anchoTripula + ANCHO_CLASE + ANCHO_POS_CLA +
...
export async function generarClasifEtapa2General({ numeroFecha, fechaEvento }, tiempos, tramos, penalizaciones, inscriptos, opciones = {}) {
  console.log('\n📄 Generando Clasificación General Etapa 2...');

  const clasificadosBase = generarClasificacionEtapa2General(tiempos, tramos, penalizaciones, inscriptos);
  const clasificados     = calcularPosicionesPorClase(clasificadosBase);

  if (clasificados.length === 0) {
    console.error('❌ No hay pilotos clasificados en Etapa 2 para generar el PDF');
    return;
  }
  ...
  const tipoPDF  = 'Clasificación General Etapa 2';
  ...
  return guardarPDF(doc, `clasif-etapa2-general-${hoy}.pdf`, opciones);
}
