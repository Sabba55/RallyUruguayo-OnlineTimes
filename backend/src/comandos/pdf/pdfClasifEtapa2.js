...
// ─── LÓGICA DE CLASIFICACIÓN (idéntica al frontend ClasFinalEtapa2) ──────────
...
/**
 * DIFERENCIA CLAVE respecto a pdfClasifFinalClases:
 * Solo devuelve los PEs de la Etapa 2 que no estén cancelados
 */
...
function pilotoCompletoTodosLosPEsEtapa2(piloto, tramos) {
...
/**
 * Penalizaciones de Etapa 2: solo las que ocurrieron en un PE de Etapa 2
 */
...
function generarClasificacionEtapa2PorClases(tiempos, tramos, penalizaciones, inscriptos) {
  const clasificados = tiempos
    .filter(p => {
      if (!pilotoCompletoTodosLosPEsEtapa2(p, tramos)) return false;
  ...
  // Agrupación con lógica RC2 / Copa RC2 (idéntica al frontend)
  const porClase = {};
...
export async function generarClasifEtapa2({ numeroFecha, fechaEvento }, tiempos, tramos, penalizaciones, inscriptos, opciones = {}) {
  console.log('\n📄 Generando Clasificación por Categoría Etapa 2...');

  const porClase            = generarClasificacionEtapa2PorClases(tiempos, tramos, penalizaciones, inscriptos);
  const categoriasOrdenadas = ordenarCategorias(Object.keys(porClase));

  if (categoriasOrdenadas.length === 0) {
    console.error('❌ No hay clases clasificadas en Etapa 2 para generar el PDF');
    return;
  }
  ...
  const tipoPDF  = 'Clasificación por Categoría Etapa 2';
  ...
  return guardarPDF(doc, `clasif-etapa2-${hoy}.pdf`, opciones);
}
