...
// Orden de prioridad de categorías (idéntico al frontend)
const ORDEN_PRIORIDAD = ['RC2', 'Copa RC2', 'RCMR', 'CT', 'RC4', 'RC3', 'RC5'];

// ─── LÓGICA DE CLASIFICACIÓN (idéntica al frontend ClasFinalClases) ──────────
...
function pilotoCompletoTodosLosPEs(piloto, peMaximo, tramos) {
  for (const pe of obtenerPEsValidos(peMaximo, tramos)) {
...
function calcularTotalPenalizaciones(nro, peMaximo, penalizaciones) {
  return penalizaciones
    .filter(pen => String(pen.nro).trim() === String(nro).trim() && (parseInt(pen.peocurrido) || 0) <= peMaximo)
...
/**
 * Genera la clasificación por clases idéntica al frontend ClasFinalClases
 */
function generarClasificacionPorClases(tiempos, tramos, penalizaciones, inscriptos) {
  ...
  // Agrupar con lógica especial RC2 / Copa RC2 (idéntica al frontend)
  const porClase = {};
...
/**
 * Dibuja la fila separadora de clase — fondo gris claro, texto negro negrita
 */
...
export async function generarClasifFinalClases({ numeroFecha, fechaEvento }, tiempos, tramos, penalizaciones, inscriptos, opciones = {}) {
  console.log('\n📄 Generando Clasificación Final por Clases...');

  const { porClase, peMaximo } = generarClasificacionPorClases(tiempos, tramos, penalizaciones, inscriptos);
  const categoriasOrdenadas    = ordenarCategorias(Object.keys(porClase));

  if (categoriasOrdenadas.length === 0) {
    console.error('❌ No hay clases clasificadas para generar el PDF');
    return;
  }

  // Todos los pilotos (para calcular anchos de columnas)
  ...
  const tipoPDF  = 'Clasificación Final por Categoría';
  const yLimite  = doc.page.height - doc.page.margins.bottom - 20;

  // Primera página: encabezado del documento + encabezado de tabla
  let y          = dibujarEncabezado(doc, { tipoPDF, numeroFecha, fechaEvento });
  let yEncTabla  = y; // guardamos Y del enc. de tabla para reusar en nuevas páginas
  ...
  categoriasOrdenadas.forEach(clase => {
    ...
    // Verificar si el banner de clase + al menos 1 fila caben en la página actual
    const espacioNecesario = ALTO_BANNER_CLASE + ALTO_FILA;
    if (y + espacioNecesario > yLimite) {
      doc.addPage();
  ...
  return guardarPDF(doc, `clasif-final-clases-${hoy}.pdf`, opciones);
}
